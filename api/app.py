...
df = df.drop(columns=["Unnamed: 0", "Model"])
...
df_scaled = scaler.fit_transform(df.drop(columns=["Name"]))
...
def recommend():
    ...
    recommended_mobiles = [
        {"Name": mobile_names[idx], **df.iloc[idx].to_dict()}
        for idx, _ in similar_mobiles
    ]

    return jsonify({
        "searched_mobile": mobile_names[mobile_index],
